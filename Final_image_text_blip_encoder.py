...
#        take only top profile_average_words words
    topn_sorted_doc = str_list[:image_average_words]
    
#    print(topn_sorted_doc)
    
    encoded_doc = np.zeros((0,512),dtype = np.single)
...
#        print(type(encoded_doc))
        encoded_doc = np.vstack((encoded_doc,text_features_np))
    
#    print('before padding size ',encoded_doc.shape)
    
    padsize = image_average_words - len(topn_sorted_doc)
    if( padsize > 0):
        pad_array = np.zeros((padsize,512),dtype = np.single)
...
#    print(encoded_doc.shape, encoded_doc[14][0] ,'\n')    
    ...
    text_encoding = encoded_doc.reshape(1, -1)
    normalized_enc = preprocessing.normalize(text_encoding)
...
#    enc = image_encoding("96573740_36 neur.jpg", wavelet = 1, rgb = 1, way1 = 1, way2 = 1)
#    enc = image_encoding("96573740_36 neur.jpg")

    uid = "749003"
    imgid = '1'

    enc = get_profile_text_enc(uid,imgid)
#    enc = image_encoding("749003_1.jpg", wavelet = 1, rgb = 1, way1 = 1, way2 = 1)
